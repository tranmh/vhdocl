...
numbering (thanks to SyntaxHighlighter by Alex Gorbatchev and the VHDL style by
Minh Cuong Tran).  The source file itself is hyperlinked, so you can edit it by
clicking on the link if you have the file associations of  your browser set
accordingly.

When parts of the source are quoted in the documentation pages, VHDocL
surrounds them by tags belonging to classes that allow markup by a user CSS
file.  (This is not currently done to the source HTML files because that would
require processing the source files a second time and increase VHDocL's runtime
accordingly.)  The following CSS classes are available:
...
Non-code words inside quoted code, such as "unnamed" notice for nameless
processes.
