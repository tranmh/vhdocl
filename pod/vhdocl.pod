...
Use JavaScript SyntaxHighlighter by Alex Gorbatchev for source HTML pages
instead of marking the syntax with CSS classes.  This will save some CPU time
generating these pages, but since VHDocL is usually limited by disk I/O, this
is not considered serious.  This option will likely be removed in the future,
so let me know if you think you need it.  (See L</BUGS AND LIMITATIONS> below
...
There is also an index.html in the output directory which links to the five
...
the HTML files which describe them in detail, located in the accordingly named
...
numbering.  This used to be done via SyntaxHighlighter by Alex Gorbatchev and
the VHDL style by Minh Cuong Tran, but is now achieved by CSS classes the
appearance of which can be overridden in the user CSS file.  The source file
itself is hyperlinked, so you can edit it by clicking on the link if you have
the file associations of  your browser set accordingly.

When parts of the source are quoted in the documentation pages, they are also
marked by tags with classes that allow markup by a user CSS file.  The
following CSS classes are available:

=over

=item B<doccom>

Text from documentation comments.
...
Identifier of the declarative item being documented.

=item B<vhdlieee>

Identifier from standard package.
...
Other VHDL declaration keyword.
...
VHDocL is (c) 2007-2011 Volker Schatz, with enhancements by Minh Cuong Tran.
It is free software and may be redistributed and/or modified under the terms of
